...
4. **Atlas économique** (`annuaire_benin.atlas`) : `aggregate` produit les comptages commune × secteur (chaque entité comptée dans sa commune majoritaire, les non-localisées comptées à part) ; `geo` apparie les 77 communes aux contours geoBoundaries (7 alias d'orthographe explicites, échec bruyant si une commune manque), les projette en chemins SVG côté Python, calcule emprises et centres, et exporte un GeoJSON nommé comme le registre ; `build` assemble une page HTML riche (recherche, filtre, liens partageables, tableau triable, deux thèmes) publiée par GitHub Pages depuis `docs/atlas/`. Aucune donnée individuelle ne sort : uniquement des agrégats.
   ...
   La carte principale est une choroplèthe Leaflet posée sur un fond OpenStreetMap (Leaflet 1.9.4 vendorisé dans `docs/atlas/vendor/`, aucun CDN ; tuiles OSM chargées à l'exécution avec attribution, inversées en thème sombre). Un masque « monde moins Bénin » (contour geoBoundaries ADM0) estompe l'extérieur du pays pour que sa silhouette ressorte du fond de carte. La seule dépendance tierce à l'exécution est le serveur de tuiles : si la couche de contours ou les tuiles manquent, le panneau et le tableau restent utilisables. Le registre ne fournissant aucune coordonnée GPS, et le projet ne publiant aucune donnée individuelle, la carte montre des densités par commune, jamais des entreprises pointées une à une. (Une vue 3D extrudée MapLibre a existé, puis a été retirée sur décision produit.)
5. **Recherche en langage naturel** en deux couches. Couche déterministe (`annuaire_benin.atlas.lexicon` + l'interpréteur de `atlas.js`) : lexique mot-clé → secteur (414 entrées) dérivé de la table de classification avec une règle de dominance à 80 % qui écarte les mots transverses, reconnaissance des 77 communes, grammaire d'intentions (compter, classer, profil, densité, spécialisation). Couche LLM (`worker/`) : un Worker Cloudflare garde la clé Gemini en secret (une page statique publique exposerait toute clé embarquée), n'accepte que l'origine de l'atlas, limite le débit par IP, et demande à gemini-2.5-flash-lite de traduire la question libre en intention structurée validée (action, secteur parmi les 25, communes). Le LLM ne produit jamais un chiffre : la page exécute l'intention sur ses agrégats. Bascule automatique : question à deux communes ou hors grammaire → Worker ; Worker indisponible → message de repli pédagogique. Toute réponse passée par l'IA est étiquetée comme telle.
