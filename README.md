...
| Entités localisées sur la carte (77 communes) | 235 256 (104 sans commune) |
| Tests | 67, tous verts |
...
- [x] **Étape 4 : atlas économique.** [Page interactive publiée](https://abiotov.github.io/annuaire-benin/atlas/) : carte choroplèthe SVG des 77 communes (contours geoBoundaries, domaine public), recherche de commune, filtre par secteur, panneau de détail avec classement et météo en direct ([Open-Meteo](https://open-meteo.com/), sans clé), vue de la commune sur fond [OpenStreetMap](https://www.openstreetmap.org/) (Leaflet vendorisé, chargé à la demande), vue tableau triable, liens partageables (état dans l'URL), thèmes clair et sombre, animations respectant `prefers-reduced-motion`. Zéro donnée individuelle (agrégats commune × secteur uniquement), rendu vérifié par captures headless dans les deux thèmes.
...
- **Pourquoi Open-Meteo et pas OpenWeatherMap :** OpenWeatherMap exige une clé API, qui serait exposée dans une page statique publique. Open-Meteo rend le même service sans clé et avec CORS ouvert. Les seuls appels externes de l'atlas (tuiles OSM, météo) sont déclenchés à la demande et se dégradent en silence hors ligne.
