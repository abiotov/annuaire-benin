...
- [x] **Étape 4 : atlas économique.** [Page interactive publiée](https://abiotov.github.io/annuaire-benin/atlas/) : choroplèthe des 77 communes **directement sur fond [OpenStreetMap](https://www.openstreetmap.org/)** (Leaflet vendorisé, pan/zoom libre jusqu'à la rue, vol animé vers la commune cliquée, contours geoBoundaries), recherche de commune, filtre par secteur, légende interactive (survoler une classe l'isole sur la carte), panneau de détail avec classement, vue tableau triable, liens partageables (état dans l'URL), icônes SVG, thèmes clair et sombre (tuiles inversées en sombre), animations respectant `prefers-reduced-motion`. Zéro donnée individuelle : agrégats commune × secteur uniquement.
